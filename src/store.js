...
const store = new Vuex.Store({
  state: {
    ...
    filterLocation: 'Add location',
    filterGuets: 'Add Guets',
    filterGuetsNumber: 0,
    data
  },
  mutations: {
    setShowSearchModal(state, data) {
      state.showSearchModal = data
    },
    setFilters (state, data) {
      state.filterLocation = data.filterLocation
      state.filterGuets = data.filterGuets
      state.filterGuetsNumber = data.filterGuetsNumber
    }
  },
  getters: {
    renderData(state) {
      ...
      state.data.forEach((item) => {
        if (state.filterGuets !== 'Add Guets') {
          if (item.maxGuests < state.filterGuetsNumber) {
            return
          }
        }
        if (state.filterLocation !== 'Add location') {
          const itemLocation = item.city + ', ' + item.country
          if (itemLocation !== state.filterLocation) {
            return
          }
        }
        data.push(item)
